...
// snapshot, test/fixtures/mcp-contract-v2.59.json (63 tools: v2.56 plus resolve_travel_mark and audit_itinerary; formerly 61:
...
const FILE = path.join(__dirname, 'fixtures', 'mcp-contract-v2.59.json');
...
(async () => {
  ...
  if (HISTORICAL) {
    ...
    cmp(`tool definitions (${now.tools.length} tools, all exactly as the v2.59 snapshot)`, now.tools, want.tools);
  ...
  console.log(fail ? `\n${fail} contract difference(s) from the ${HISTORICAL ? 'historical submission' : 'v2.59 snapshot'}.`
    : (HISTORICAL ? '\nHistorical comparison: nothing removed; OAuth, discovery and shared result shapes unchanged.' : '\nMCP contract matches the v2.59 snapshot.'));
